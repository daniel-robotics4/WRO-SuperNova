...
int pasilloAbierto = 30; // Variable para decidir si un pasillo esta abierto o no.
int pasilloCerrado = 20; // Variable para decidir si un pasillo esta cerrado o no.
...
switch (currentState){
  ...
  case DECIDIR_SENTIDO:
    ...
    if (distanceRight > pasilloAbierto && distanceLeft < pasilloCerrado) {
    ...
    if (distanceLeft > pasilloAbierto && distanceRight < pasilloCerrado) {
...
//Ejecuta la secuencia de giro a la izquierda
  case GIROLEFT:    
    // codigo para giro a la izquierda
    detenido(); 
    retrocede(150);
    delay(2000);
    girarizq();
    avanza(200);
    Serial.println("GIRANDO IZQUIERDAAAAA");
    delay(2000);
...
// Ejecuta la secuencia de giro a la derecha
   case GIRORIGHT:
    // codigo para codigo a la derecha
    detenido(); 
    retrocede(150);
    delay(2000);
    girarder();
    avanza(200);
    Serial.println("GIRANDO DERECHAAAAAA");
    delay(2000);
    //currentState = STRAIGHT; // Vuelve a navegar recto después del giro
    currentState = STOPPED; 
    break;

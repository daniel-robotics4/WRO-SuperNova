...
void setup() {   //  función que solo se ejecutará una sola vez al inicio del programa
  ...
  // ---------------- Configuración de de la terminal (OPEN----------------
  Serial.begin(115200);   // Iniciando comunicación serial para depuración
 ...
 // ------------------------- Caso Para Que Avance El Robot (OPEN) -------------------------
  ...
  case STRAIGHT:
    ...
    if (straightStartTime == 0) {
        straightStartTime = millis(); // Inicia el contador al entrar al case
        delay(15);
    }

    unsigned long elapsed = millis() - straightStartTime;

    if (elapsed > 2000) { // Más de 2 segundos
        if ((distanceFront < frontObject) && (distanceFront != MAX_DISTANCE) && (sense == 1 && 40 < distanceFront)) {
            currentState = LEFT;
            straightStartTime = 0; // Reset contador al cambiar de estado
            delay(15);
            break;
        }
    ...
    } else { // Menos o igual a 2 segundos
        // Prioridad 2: Corrección lateral para mantenerse centrado
        if ((distanceRight != MAX_DISTANCE) && (distanceLeft != MAX_DISTANCE && (maximumCorrections < 2))) {
...
// Serial.print("Rsmall COMFIRMADO"); 
  bool done = false;
  while (!done) {
    done = move.backward(move.mmToPulses(180, diameter, ppr));
    delay(5);
  }
  servoRigth();
  done = false;
  while (!done) {
    done = move.forward(move.mmToPulses(700, diameter, ppr));
    delay(5);
  }
  servoCenter();
}
...
// Serial.print("Rbig COMFIRMADO");
  bool done = false;
  while (!done) {
    done = move.backward(move.mmToPulses(300, diameter, ppr));
    delay(5);
  }
  servoRigth();
  done = false;

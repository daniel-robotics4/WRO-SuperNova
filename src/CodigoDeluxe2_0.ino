...
int objetoDelante = 70; //Si el sensor frontal detecta a menos de esto, es hora de decidir un giro
...
switch (currentState){
 ...
 if (distanceFront < objetoDelante && distanceFront != MAX_DISTANCE && sentido != 0) {
  ...
  case DECIDIR_SENTIDO:
    ...
    if (distanceLeft < pasilloCerrado && distanceLeft < distanceRight)   {
    ...
    if (distanceRight < pasilloCerrado && distanceRight < distanceLeft) {

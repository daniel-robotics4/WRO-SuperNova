...
#define TRIGGER_PIN3  A4  // Pin para el sensor ultrasonico 3 Front
#define ECHO_PIN3     A5 // Pin para el sensor ultrasonico 3 Front
...
AF_DCMotor motor(2);      // Motor conectado al puerto M2
...
int objetoDelante = 50; //Si el sensor frontal detecta a menos de esto, es hora de decidir un giro
...
Serial.println(encoder.getEncoderCount());
 ...
 switch (currentState) {
  ...
  case SENTIDO: Serial.println("SENTIDO"); break;
 }
...
switch (currentState){
  ...
  case INICIAL:
    ...
    currentState = STOPPED ; // Pasa al estado de navegación
    break;
...
void eAvanza(int cm) {
  ...
  avanza(80);
  readUltrasonicSensors();
  if ((encoder.getEncoderCount() - ticksIniciales) < ticksObjetivo) {

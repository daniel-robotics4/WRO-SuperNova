...
enum CarState {
 ...
 CORRECCIONLEFT,   // Estas muy a la izquierda
 CORRECCIONRIGHT    // Estas muy a la derecha
};
...
// Mostrar el estado actual en el monitor serial para depuración
  ...
  else if (currentState == CORRECCIONLEFT) Serial.println("CORRECCIONLEFT");
  else if (currentState == CORRECCIONRIGHT) Serial.println("CORRECCIONRIGHT");
...
switch (currentState){
  ...
  default:
// Lógica optimizada usando variable Switch y switch-case
  if ((distanceRight > 70) && (distanceLeft < 25)) {
    currentState = CORRECCIONLEFT;
  } else if ((distanceLeft > 70) && (distanceRight < 15)) {
    currentState = CORRECCIONRIGHT;
  }
    break;

...
int wallfence = 5; // Estas muy cerca de la pared
int laps = 0; // 7
...
break;
 ...
 // ------------------------- Caso Para Que Avance El Robot (OPEN) -------------------------
  ...
  case STRAIGHT:
   ...
   if (distanceFront < frontObject && distanceFront != MAX_DISTANCE && sense == 1) {
      currentState = LEFT;
      break; // Salir del switch para procesar el nuevo estado
    }

    if (distanceFront < frontObject && distanceFront != MAX_DISTANCE && sense == 2) {
      currentState = RIGHT;
      break; // Salir del switch para procesar el nuevo estado
    }
...
void Rsmall() {
  ...
  bool done = false;
  while (!done) {
    done = move.backward(move.mmToPulses(500, diameter, ppr));
    delay(5);
  }
  servoRigth();
  done = false;
  while (!done) {
    done = move.forward(move.mmToPulses(680, diameter, ppr));
    delay(5);
  }
  servoCenter();
}

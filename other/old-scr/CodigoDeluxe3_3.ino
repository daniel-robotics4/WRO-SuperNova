...
break;
 ...
 // ------------------------- Caso Para Que Avance El Robot (OPEN) -------------------------
  ...
  case STRAIGHT:
   ...
   if (distanceFront < frontObject && distanceFront != MAX_DISTANCE && sense == 1  && 100 < distanceLeft) {
      currentState = LEFT;
      break; // Salir del switch para procesar el nuevo estado
    }

    if (distanceFront < frontObject && distanceFront != MAX_DISTANCE && sense == 2 && 100 < distanceRight.) {
      currentState = RIGHT;
      break; // Salir del switch para procesar el nuevo estado
    }
